...
public interface ClientDao {
    ...
    // additional_information, autoapprove) values(#{clientId}, #{resourceIds}, #{clientSecret},#{clientSecretStr},
    // #{scope}, #{authorizedGrantTypes}, #{webServerRedirectUri}, #{authorities}, #{accessTokenValidity},
    ...
    // ,web_server_redirect_uri webServerRedirectUri from oauth_client_details t where t.id = #{id}")
    Client getById(Long id);

    // @Select("select * from oauth_client_details t where t.client_id = #{clientId}")
    Client getClient(String clientId);

    // @Update("update oauth_client_details t set t.client_secret = #{clientSecret},t.client_secret_str =
    // #{clientSecretStr} where t.id = #{id}")
    ...
    int saveClientPermission(Long clientId, List<Long> permissionIds);
